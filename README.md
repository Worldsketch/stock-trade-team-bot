...
### 적립식 기본매수 + DCA (357)
- 기본 적립: `DAILY_FIXED_BUY_QTY`(기본 1주)로 하루 1회 매수
- 진입 우선순위:
  - ET `10:30~15:30` 신호 구간에서 매수 시도
  - 신호 미충족 시 ET `15:40` 이후 보장 매수
- 신호 조건(최소 `DAILY_BASE_MIN_SIGNALS`, 기본 2개 충족):
  - 전일종가 대비 하락(`DAILY_BASE_DROP_TRIGGER`, 기본 -0.3%)
  - 현재가 `<=` intraday VWAP
  - 5분봉 RSI `<= DAILY_BASE_RSI_MAX`(기본 45)
- 급등 추격 방지: 당일 상승률이 `DAILY_BASE_PUMP_BLOCK`(기본 +1.5%) 이상이면 기본 적립 보류
- **기본 적립도** 현금 바닥선/일일 매수 상한을 동일하게 적용

전일종가 대비 당일 하락률 기준 DCA 비율:
...
### 자동 부분익절
- 수익률 구간 도달 시 분할 매도 (`AUTO_TAKE_PROFIT_LEVELS`, 기본 `12/20/30%`)
- 구간별 매도 비중 (`AUTO_TAKE_PROFIT_SELL_PCTS`, 기본 `15/20/25%`)
- 추격형 지정가로 미체결 재호가 후 체결 관리

### 자동 모드 전환
- 예수금 비중 ≤ 45% 또는 SMA200 하회 2종목 이상 → 방어 모드
...
- **봇 루프 구동 시간**: 미국장(ET 04:00~20:00) 또는 데이장(KST 09:00~16:00)
- **자동 매수 전략 시간**: 미국장 + 데이장에서 실행 (세션에 따라 일반/주간거래 주문 분기)
- **자동 매도(부분익절/트레일링) 시간**: 정규장(ET 09:30~16:00)에서만 실행
...
- **Graceful Shutdown**: 서버 종료 시 봇 루프를 안전 중단하고 포지션은 유지
...
- 전략/거래시간
  - 자동 전략 실행 시간 가드를 `거래 가능 세션(미국장+데이장)` 기준으로 통합
  - 기본 적립(하루 1주)도 `현금 바닥선/일일 매수 상한`을 동일 적용하도록 보정
- 체결 정산
  - 수동/자동 매도 추격 주문에서 미체결 목록 소멸 시 `전량체결 가정` 제거
  - 주문 전 보유수량 대비 변화량으로 체결수량을 추정해 `trade_log` 손익 왜곡 가능성 완화
